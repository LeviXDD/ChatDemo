...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [deleteButton setImage:[UIImage imageNamed:@"close_share"] forState:UIControlStateHighlighted];
        [deleteButton setImage:[UIImage imageNamed:@"close_share"] forState:UIControlStateNormal];
...
//                [self selectEmotion:btn.emotion];
                InputView *inputView= (InputView *)self.superview.superview.superview.superview;
                if (inputView != nil && [inputView isKindOfClass:[InputView class]]) {
                    [inputView inputEmotion:btn.emotion];
                }
            }
            break;
...
- (void)setEmotions:(NSArray *)emotions
{
    ...
    for (int i = 0; i<count; i++) {
        HWEmotionButton *btn = [HWEmotionButton buttonWithType:UIButtonTypeCustom];
        [self addSubview:btn];
...
- (void)layoutSubviews
{
    ...
    CGFloat btnH = (self.height - 2 * inset) / HWEmotionMaxRows;
    for (int i = 0; i<count; i++) {
        UIButton *btn = self.subviews[i + 1];
        btn.width = btnW;
        btn.height = btnH;
        btn.left = inset + (i%HWEmotionMaxCols) * btnW;
        btn.top = inset + (i/HWEmotionMaxCols) * btnH;
    }
...
- (void)deleteClick
{
//    [RHNotificationCenter postNotificationName:HWEmotionDidDeleteNotification object:nil];
}
...
- (void)selectEmotion:(HWEmotion *)emotion
{
   ...
   // [HWEmotionTool addRecentEmotion:emotion];
    ...
    InputView *inputView= (InputView *)self.superview.superview.superview.superview;
    if (inputView != nil && [inputView isKindOfClass:[InputView class]]) {
        [inputView inputEmotion:emotion];
    }
    // 发出通知
//    NSMutableDictionary *userInfo = [NSMutableDictionary dictionary];
//    userInfo[HWSelectEmotionKey] = emotion;
//    [RHNotificationCenter postNotificationName:HWEmotionDidSelectNotification object:nil userInfo:userInfo];
}
@end
